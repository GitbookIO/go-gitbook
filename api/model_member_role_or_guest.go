...
func (dst *MemberRoleOrGuest) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(MemberRoleOrGuest)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(MemberRoleOrGuest)")
	}
}
...
type NullableMemberRoleOrGuest struct {
	value *MemberRoleOrGuest
	isSet bool
}

func (v NullableMemberRoleOrGuest) Get() *MemberRoleOrGuest {
	return v.value
}

func (v *NullableMemberRoleOrGuest) Set(val *MemberRoleOrGuest) {
	v.value = val
	v.isSet = true
}
...
func NewNullableMemberRoleOrGuest(val *MemberRoleOrGuest) *NullableMemberRoleOrGuest {
	return &NullableMemberRoleOrGuest{value: val, isSet: true}
}
